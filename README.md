...
3. コード品質と実装原則
   ...
   - 最小限かつ効率的な実装を心がける:
     ...
     - 共通の検証ロジックは関数化する（例: `checkArrayBufferConstructorAvailability`など）
   ...
   - ESLintセレクタを活用して効率的に実装する:
     - シンプルなパターンマッチには、`"NewExpression[callee.name='ArrayBuffer']"`のようなセレクタを使用する
     - セレクタと型チェックを組み合わせて、直接的/間接的なすべての使用パターンをカバーする
   - 型チェックを活用して堅牢な実装を行う:
     - `createIsTargetType`などの関数を使用して型チェックを抽象化する
     - 変数に代入されたコンストラクタや、複雑な参照パターンも正確に検出する
   - コードのフォーマットと読みやすさに配慮する:
     - 長い条件式や関数呼び出しは複数行に分割し、可読性を高める
     - 命名は明示的で、目的が明確なものを選ぶ（例: `checkArrayBufferConstructorAvailability`）
