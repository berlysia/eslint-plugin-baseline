...
tester.run(seed.concern, rule, {
	valid: [
		{
			code: "const arr = [1, 2, 3]; const found = arr.findLast(num => num > 1);",
			options: [{ asOf: "2025-03-01", support: "widely" }],
		},
		{
			code: "const users = [{id: 1, name: 'Alice'}, {id: 2, name: 'Bob'}]; const user = users.findLast(u => u.id === 2);",
			options: [{ asOf: "2025-03-01", support: "widely" }],
		},
		{
			code: "function findInArrayReverse(arr, predicate) { return arr.findLast(predicate); }",
