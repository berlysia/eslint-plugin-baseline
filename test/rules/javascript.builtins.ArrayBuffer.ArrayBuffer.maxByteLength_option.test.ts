...
createSimpleRuleTest({
	...
	validOnlyCodes: [
		...
		"const ArrayBuffer = class MyArrayBuffer { constructor(size: number, options: { maxByteLength?: number }) { return { byteLength: size }; } }; const buffer = new ArrayBuffer(10, { maxByteLength: 32 });",
		// 名前が似ているが別のオブジェクト
		"const customBuffer = { constructor: function(size, options) { return { byteLength: size }; } }; customBuffer.constructor(10, { maxByteLength: 20 });",
		...
		"const ArrayBuffer = class MyArrayBuffer { constructor(size: number, options: { maxByteLength?: number }) { return { byteLength: size }; } }; const MyArrayBuffer2 = class extends ArrayBuffer { constructor(size: number, options: any) { super(size, { ...options, maxByteLength: 32 }); } }; const buffer = new MyArrayBuffer2(16, { myOpts: true });",
		// extendsされている場合のsuperの呼び出しでの利用だがArrayBufferという名前の異なるクラス
		"const ArrayBuffer = class MyArrayBuffer { constructor(size: number, options: { maxByteLength?: number }) { return { byteLength: size }; } }; const MyArrayBuffer2 = class extends ArrayBuffer { constructor(size: number, options: any) { super(size, options); } }; const buffer = new MyArrayBuffer2(16, { maxByteLength: 32 });",
	],
	validOption: {
		asOf: "2024-07-10",
		support: "newly",
