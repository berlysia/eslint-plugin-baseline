...
- 段階的な機能拡充とリファクタリング:
  - 単純なパターンから始め、必要に応じて機能を徐々に強化する
  - 実験的な実装が安定したら、既存コードに統合して重複を排除する
  - コードの重複は技術的負債となるため、早期に解消する
...
2. **多様なJavaScriptパターンのテスト**

   - 基本的なメソッド呼び出し: `obj.method()`
   - 計算プロパティ: `obj["method"]()`、`obj[prop]()`
   - プロトタイプ経由: `Constructor.prototype.method.call(obj)`
   - 分割代入パターン: `const { method } = obj; method()`
   - 変数経由の呼び出し: `const fn = obj.method; fn()`
   - 配列リテラルメソッド: `[].method.call(obj)` (Array特有)
   - 関数内の呼び出し: `function process(obj: Type) { obj.method() }`
   - 型注釈付きパラメータ: バリデータが型情報を活用できるよう、関数パラメータには型注釈を付ける
...
4. **テスト条件の設定**
   ...
   - 検出すべきパターンは`codes`に含める
   - 検出すべきでないパターンは`validOnlyCodes`に含める
...
既存の検証器（`createInstanceMethodValidator`など）で対応できない複雑なパターンが見つかった場合、以下のプロセスで強化します：

1. **問題の特定と分析**
   - 既存の検証器でカバーできないパターンを特定
   - AST解析ツールを使用して、対象パターンのAST構造を理解
   - 必要な拡張機能を明確化
...
3. **段階的な強化アプローチ**
   - まず既存の検証器の機能を拡張して対応可能か検討
   - 大幅な変更が必要な場合は、一時的に別の強化版を作成して検証
   - 検証が完了し安定したら、機能を統合して重複を排除

4. **型情報の活用**
   - 複雑なパターン検出には型情報を最大限活用
   - 関数パラメータには適切な型注釈を付けてテスト

このプロセスにより、コードの一貫性と保守性を維持しながら、段階的にバリデータを強化できます。
