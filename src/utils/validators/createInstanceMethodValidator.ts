...
export function createInstanceMethodValidator({
	...
	>(
		...
		const methodReferences = new Map<
			string,
			{ originNode: TSESTree.Node; fromType: string }
		>();
		 ...
		 * @returns ユニークな識別キー
		 */
		function getArrayLiteralCallKey(
		 ...
		 */
		function trackArrayLiteralMethod(
			memberExpr: TSESTree.MemberExpression,
		): void {
		 ...
		 */
		function isTrackedArrayLiteralMethod(
			memberExpr: TSESTree.MemberExpression,
		): boolean {
			const key = getArrayLiteralCallKey(memberExpr);
		...
		return {
				...
				if (
					...
					// 通常のメソッド参照
						...
						(node.init.property.type === "Identifier" &&
							node.init.property.name === methodName) ||
						// obj["method"]
			...
			ObjectPattern(node: TSESTree.ObjectPattern) {
				...
				// 分割代入元が対象インスタンスかチェック
				const isFromTargetInstance = sharedValidator.validateInstanceType(
					parent.init,
				);
			...
			MemberExpression(node: TSESTree.MemberExpression) {
				...
				if (typeName === "Array" && isArrayLiteralMethod(node)) {
					...
					if (
						...
						(node.parent.property.name === "call" ||
							node.parent.property.name === "apply")
					) {
				...
				else if (isStringLiteralWithValue(node.property, methodName)) {
					// インスタンスメソッド: instance["method"]
					if (sharedValidator.validateInstanceType(node.object)) {
						reportOnce(node);
					}
			...
			"CallExpression[callee.type='MemberExpression']"(
				...
				if (
					...
					if (
						...
						if (
							...
							(methodRef.fromType === "array_literal" ||
								sharedValidator.validateInstanceType(
					...
					// 直接的なメソッド呼び出し
						...
						if (
							typeName === "Array" &&
							isTrackedArrayLiteralMethod(callee.object)
						) {
						...
						// 標準的なメソッド呼び出し: obj.method.call(obj, args) / proto.method.call(obj, args)
							...
							// 他のメソッド参照経由の呼び出し
								...
								sharedValidator.validateInstanceType(callee.object.object)
							) {
								reportOnce(callee.object);
							}
						}
