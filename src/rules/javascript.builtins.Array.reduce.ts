...
export const seed = createSeed({
  ...
  mdnUrl: "https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Array/reduce",
  specUrl: "https://tc39.es/ecma262/multipage/indexed-collections.html#sec-array.prototype.reduce",  
...
const rule = createRule(seed, {
  create(context) {
    ...
    // Check if the type is Array or ArrayLike
    const isArrayType = createIsTargetType(typeChecker, "Array");

    return {
      // Look for method calls with 'reduce' property name
      CallExpression(node) {
        if (node.callee.type === "MemberExpression") {
          const property = node.callee.property;
          if (property.type === "Identifier" && property.name === "reduce") {
