...
export const seed = createSeed({
	concern: "Array[Symbol.species]の使用",
	compatKeys: ["javascript.builtins.Array.@@species"],
	mdnUrl:
		"https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Array/Symbol.species",
	specUrl:
...
const rule = createRule(seed, {
	create(context) {
		...
		return {
			// Symbol.speciesの直接アクセスを検出（例：Array[Symbol.species]）
				...
				if (
					...
					if (classNode?.superClass) {
						...
						if (
							...
							classNode.superClass.name === "Array"
						) {
							checkAvailability(node);
							return;
						}
			...
			// Array継承クラスでのthis.constructorの使用検出
