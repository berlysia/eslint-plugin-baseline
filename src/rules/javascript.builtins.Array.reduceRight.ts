...
export const seed = createSeed({
  ...
  compatKeys: ["javascript.builtins.Array.reduceRight"],
...
const rule = createRule(seed, {
  create(context) {
    ...
    return {
      // Look for method calls with 'reduceRight' property name
      CallExpression(node) {
        if (node.callee.type === "MemberExpression") {
          const property = node.callee.property;
          if (property.type === "Identifier" && property.name === "reduceRight") {
